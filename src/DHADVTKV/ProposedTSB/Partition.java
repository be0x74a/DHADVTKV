...
class Partition {
    ...
    void transactionalGet(TransactionalGet request) {
        ...
        DataObject snapshotConsistentContent = selectSnapshotConsistentVersion(objectVersions, request.getSnapshot());
        ...
        if (version == UNDEFINED) {
            long tentativeVersion = snapshotConsistentContent.getMetadata().getOrDefault("tentativeVersion", UNDEFINED);
            if (tentativeVersion <= request.getSnapshot()) {
                long transactionID = snapshotConsistentContent.getMetadata().getOrDefault("transactionID", UNDEFINED);
                this.pendingTransactionalGets.computeIfAbsent(transactionID, k -> new ArrayList<>()).add(request);
            }
        } else {
            Channel.sendMessage(new TransactionalGetResponse(request.getTo(), request.getFrom(), snapshotConsistentContent));
        }
    }
    ...
    void transactionValidation(TransactionValidation request) {
        if (request.isConflicts()) {
            ...
            kv.commitTentativeVersions(request.getTransactionID(), request.getPutKeys(), request.getLsn());
        }

        for (TransactionalGet getRequest : pendingTransactionalGets.getOrDefault(request.getTransactionID(), new ArrayList<>())) {
