...
set xtics 20\n
plot 'tests/$1_$2_$3_$4_tpc.dat' with linespoints linestyle 1, \
     'tests/$1_$2_$3_$4_plb.dat' with linespoints linestyle 2\n
pause -1 'Hit any key to continue'\n"

echo -e ${gnuplotScript} > temp.gnuscript
cat /dev/null > tests/$1_$2_$3_$4_tpc.dat
cat /dev/null > tests/$1_$2_$3_$4_plb.dat
...
do
	...
	echo "`expr $i - 8` $(tail -1 temp.out)" >> tests/$1_$2_$3_$4_tpc.dat
	rm temp.config
	rm temp.out
	templateFillSize="TEMPLATE_SIZE $i\n"
	echo -e ${templateFill}${templateFillSize} | cat - configs/config-plb-template.txt > temp.config
	java -classpath ./out/production/Sim:./jep-2.3.0.jar:./djep-1.0.0.jar peersim.Simulator temp.config > temp.out 2>/dev/null
	echo "`expr $i - 8` $(tail -1 temp.out)" >> tests/$1_$2_$3_$4_plb.dat
	rm temp.config
	rm temp.out
done
